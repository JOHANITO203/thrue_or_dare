...
export default function MainMenu({ onNewGame }) {
  ...
  const avatars = [
    { 
      ...
      finalAngle: 0, 
      startAngle: -720,  // 2 tours complets depuis la gauche
      midAngle: -360,    // 1 tour au milieu
      delay: '0.1s' 
    },
    { 
  ...
  return (
    <div className={`glass-premium w-[520px] max-w-full overflow-hidden p-8 flex flex-col items-center gap-6 text-center transition-all duration-1000 ${
      ...
      {/* Titre avec icône flamme - Animation séquentielle */}
      ...
      {/* Cercle d'avatars avec animation spirale spectaculaire - FIX */}
      <div 
        className="relative w-full h-[300px] mt-2" 
        style={{ 
          animationDelay: '0.4s',
          maxWidth: '400px',  // Contrainte pour ne pas dépasser
          margin: '0 auto'
        }}
      >
        ...
        {/* Effet de particules au centre après la spirale */}
        {spiralComplete && (
      ...
      {/* Boutons du menu avec particules et glass effects */}
      <div className="w-full flex flex-col gap-4 mt-4 animate-fade-scale-in" style={{ animationDelay: '3s', animationFillMode: 'both' }}>
        ...
        {/* Bouton Paramètres */}
        <button className="w-full rounded-full py-4 font-semibold text-white bg-gradient-to-b from-[#5CD96E] to-[#34C759] shadow-lg hover-lift button-particles relative overflow-hidden group">
          <span className="relative z-10">⚙️ Paramètres</span>
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>
        </button>
        
        {/* Bouton Premium Content */}
        <button className="w-full rounded-full py-4 font-semibold text-white bg-gradient-to-b from-[#FF6AC0] to-[#FF3B72] shadow-lg hover-lift button-particles relative overflow-hidden group">
